...
print('method', (carla.Vehicle.__module__))
print('method', allccc(carla.Vehicle))
print('member', members(carla.Vehicle))

print('method', methods(carla.VehicleControl))
print('member', members(carla.VehicleControl))
'''
...
class World(object):
    ...
    def restart(self):
        ...
        self._rgb = CameraSaver(self.player, self.hud)
        self._rgb.set_sensor(0)
        self._rgb.set_name('rgb')
        self._seg = CameraSaver(self.player, self.hud)
...
class KeyboardControl(object):
    ...
    def _parse_keys(self, keys, milliseconds):
        ...
        self._control.throttle = max(0.1, self._control.throttle)
...
class CollisionSensor(object):
    ...
    def _on_collision(weak_self, event):
        ...
        self._hud.notification('Collision with %r' % actor_type + ' Restart !!')
...
class CameraSaver(object):
    def __init__(self, parent_actor,hud):
        ...
        self._image = np.zeros((600, 800, 3))
        self._index = 0
        self._name = 'no name'
        self._frame_num = 0
        world = self._parent.get_world()
    ...
    def save_img(self) :
        filename = '_'+self._name+'/' + ('%8d' % self._frame_num)+'.png'
...
class CameraManager(object):
    def __init__(self, parent_actor, hud):
        ...
        self._camera_transforms = [
            ...
            carla.Transform(carla.Location(x=-0.1, y = -0.3, z=1.35), carla.Rotation(pitch = -3))]
...
def game_loop(args):
    ...
    xxjoystick = pygame.joystick.Joystick(0)
    xxjoystick.init()
    name = xxjoystick.get_name()
    print("name : ", name)
    ...
    try:
        ...
        #remove all traffic.light and traffic.speedlimitsign
        actor_list = world.world.get_actors()
        #print(actor_list) 
        for xx in actor_list :
            print(xx.id , xx.type_id)
        print('len(actor_list)',len(actor_list))
        print(len(actor_list.filter('vehicle.*')))
        '''
        for xx in actor_list.filter('traffic.*'):
            try:
                print('des' ,xx.id, xx.type_id )
                xx.destroy()
            except:
                continue
        '''
        ...
        def try_spawn_random_vehicle_at(transform, is_auto = 1):
            #print(transform)
            blueprint = blueprints[19]
            if blueprint.has_attribute('color'):
                color = random.choice(blueprint.get_attribute('color').recommended_values)
                blueprint.set_attribute('color', color)
            vehicle = world.world.try_spawn_actor(blueprint, transform)
        ...
        spawn_points = list(world.world.get_map().get_spawn_points())
