...
class suspend_simulation(object):
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(suspend_simulation._get_packed_fingerprint())
    ...
    def decode(data):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != suspend_simulation._get_packed_fingerprint():
            raise ValueError("Decode error")
        return suspend_simulation._decode_one(buf)
    ...
    def _get_packed_fingerprint():
        if suspend_simulation._packed_fingerprint is None:
            suspend_simulation._packed_fingerprint = struct.pack(">Q", suspend_simulation._get_hash_recursive([]))
        return suspend_simulation._packed_fingerprint
